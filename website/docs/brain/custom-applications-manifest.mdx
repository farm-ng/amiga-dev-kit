...
```bash
journalctl -xef | grep -E 'Type must be a ManifestServiceType or string|JSON data was not parsable|JSON data is not a dictionary|Invalid service type|Services is not a dictionary|Service dep for|depends on itself|, a non-existent service|, which is not a service|Directory state file not found, no users loaded|Directory state file failed to parse JSON|Directory state JSON file is not dictionary, does not contain users dictionary'
```
