...
the `install.sh`, `uninstall.sh` and `manifest.json` files in the root of the package.
...
Later, in the implementation of the coroutine [`stream_camera`](https://github.com/farm-ng/camera-streamer-kivy/blob/2d701d8e79ca331e1afe0c9838a562a43371e2f4/src/main.py#L85C3-L102C25), we have to create a new `EventClient` to leverege the new `subscribe` API to subscribe to the camera events asynchronosly:
