...
1. Suppose `farm-ng-user-kyle` is the owner who registered the app.
2. Another user, `farm-ng-user-edgar`, needs to make changes to the app.
3. Edgar can switch to Kyle's account by running the following command in the terminal:
    ```bash
    sudo -su farm-ng-user-kyle
    ```
4. This command switches Edgar to Kyle's user account, granting him the necessary control.
