...
This step is required in order to have your custom application appear on the app screen.
The file stores the path to your code base to direct the brain when your app icon is selected.

There are two edits to be made to the file, both are in CAPS. TEMPLATE_NAME and YOUR_USER_HERE
...
To do this, open a terminal and navigate to the directory where you cloned the
[**amiga-app-template-kivy**](https://github.com/farm-ng/amiga-app-template-kivy) repository.

```bash
cd <to-your-app-directory>DISPLAY=:0 ./entry.sh  # install and run
```
and run:

```bash
DISPLAY=:0 ./entry.sh  # to run
```
