...
So we will have a box layout that stacks 2 sub-widgets horizontally (by default) of:
...
You should now see camera stream to the right of the the `AmigaTpdo1` values from the canbus.
...
The `VirtualJoystickApp` inherits from the kivy `App` class, so it has all the features of a generic `App` and anything we add to it.
