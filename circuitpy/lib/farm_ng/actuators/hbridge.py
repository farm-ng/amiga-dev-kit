...
class HBridgeController:
    def __init__(self, main_loop: MainLoop):
        ...
        self.dir = 0
        self.repeater = TickRepeater(ticks_period_ms=100)

        self.status_repeater = TickRepeater(ticks_period_ms=500)
        ...
        self.stop_message = Message(
            id=0x18EF7000, data=pack('>3BHHB', 0xA2, 0x00, 0x00, frequency, duty_cycle, 0xFF), extended=True
        )
        self.reverse_message = Message(
            id=0x18EF7000, data=pack('>3BHHB', 0xA2, 0x00, 0x02, frequency, duty_cycle, 0xFF), extended=True
        )
        self.forward_message = Message(
            id=0x18EF7000, data=pack('>3BHHB', 0xA2, 0x00, 0x01, frequency, duty_cycle, 0xFF), extended=True
        )
        self.reset_fault_message = Message(
            0x18EF7000, data=pack(">8B", 0xA6, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF), extended=True
        )

        self.request_output_status_message = Message(
            0x18EF7000, data=pack(">8B", 0xD0, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF), extended=True
        )
        self.request_output_current_message = Message(
            0x18EF7000, data=pack(">8B", 0xD3, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF), extended=True
        )

        self.request_fault_message = Message(
            0x18EF7000, data=pack(">8B", 0xB6, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF), extended=True
        )
        self.configure_current_message = Message(
            0x18EF7000,
            data=pack(">4BBHB", 0xA1, 0x00, continuous_limit, 0xFF, continuous_delay, inrush_limit, inrush_delay),
            extended=True,
        )
